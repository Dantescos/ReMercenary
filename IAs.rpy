    ...
    class IA_Mago:
        ...
        def evaluar(self, objeto, enemigo):
            ...
            if magia < 200 and cooldown == 0:
                print("El Mago recupera magia.")
                return objeto.recuperar_magia(250)

            if magia >= 500 and cooldown == 0:
                dado = random.randint(1, 4)
                if dado == 1:
                    return objeto.vientos_infernales_prohibidos(enemigo)
                elif dado == 2:
                    return objeto.ataque_magico_prohibido(enemigo)
                elif dado == 3:
                    return objeto.ataque_castigo_divino(enemigo)
                else:
                    return objeto.bola_fuego_mejorada(enemigo)

            if magia >= 200 and magia < 500 and cooldown == 0:
                dado = random.randint(1, 3)
                if dado == 1:
                    return objeto.ataque_hielo_infernal(enemigo)
                elif dado == 2:
                    print("Mago usa Bola de Fuego Mejorada.")
                    return objeto.bola_fuego_mejorada(enemigo)
                else:
                    return objeto.ataque_basico(enemigo)
